...
Ptr<OutputStreamWrapper> fixedShareStream;
...
static void
FixedShareRttTracer (Time oldval, Time newval)
{
  if (firstFIxedShareRtt)
    {
      //*rrttStream->GetStream () << "0.0 " << oldval.GetSeconds () << std::endl;
      firstFIxedShareRtt = false;
    }

    *fixedShareStream->GetStream ()<< Simulator::Now ().GetSeconds () << "," << newval.GetSeconds () << std::endl;
}
...
TraceRealRtt (std::string rrtt_tr_file_name)
{
  ...
  Config::ConnectWithoutContext ("/NodeList/2/$ns3::TcpL4Protocol/SocketList/*/realRTT", MakeCallback (&RealRttTracer));
}
static void
...
TraceDelta (std::string delta_tr_file_name)
{
  ...
  Config::ConnectWithoutContext ("/NodeList/2/$ns3::TcpL4Protocol/SocketList/*/Delta", MakeCallback (&DeltaTracer));
}


/************************tracing for fixed share rtt*******************/
static void
TraceFixedSharedExpertRTT (std::string expert_rtt_tr_file_name)
{
  AsciiTraceHelper ascii;
  fixedShareStream = ascii.CreateFileStream (expert_rtt_tr_file_name.c_str ());
  Config::ConnectWithoutContext ("/NodeList/2/$ns3::TcpL4Protocol/SocketList/0/FixedShare", MakeCallback (&FixedShareRttTracer));
}
...
TraceRtt_2 (std::string rtt_tr_file_name_2)
{
  ...
  Config::ConnectWithoutContext ("/NodeList/2/$ns3::TcpL4Protocol/SocketList/*/RTT", MakeCallback (&RttTracer_2));
}
...
int main (int argc, char *argv[])
{
  ...
  std::string rtt_tr_file_name = "dumbbell_RTT.trace";
  std::string rto_tr_file_name = "";
  std::string rttvar_tr_file_name = "";
  std::string rrtt_tr_file_name = "dumbbell_realRTT.trace";
  std::string delta_tr_file_name = "dumbbell_delta.trace";

  std::string fixed_share_rtt_tr_file_name = "dumbbell_fixedShareRTT.trace";
  ...
  if (transport_prot.compare ("TcpTahoe") == 0)
    ...
    int nodes_on_each_side = (num_nodes/2)-1;
    ...
    //NodeContainer leftGateway = leftGate.Get(0);
    ...
    leftDevices = wifi.Install(phy, mac, sources);
    ...
    stack.Install (leftGate);
    stack.Install (sinks);
    stack.Install (rightGate);
    ...
    */
    ...
    //p2pInterfaces = address.Assign (gates);
      ...
      */
      ...
      for (uint16_t i = 0; i < num_flows; i++)
        ...
        std::cout<<"Reached here\n";
          ...
          if (tracing)
            {
              ...
              if (tr_file_name.compare ("") != 0)
                {
                  ...
                  stack1.EnableAsciiIpv4All (ascii_wrap);
                }
              ...
              /**************Fixed Share Rtt Estimation*********************/
              if (fixed_share_rtt_tr_file_name.compare ("") != 0)
                  {
                    Simulator::Schedule (Seconds (trace_start), &TraceFixedSharedExpertRTT, fixed_share_rtt_tr_file_name);
                  }
          ...
          xmlName<<"dumbbell_wired_nodes"<< num_nodes<< "_flows"<< num_flows <<".flowmonitor";
