...
RttMeanDeviation::FloatingPointUpdate (Time m)
{
  ...
  //double meas = m.ToDouble(Time::S);

  
  //m_SampledRTT = m;

  //double delta = m.ToDouble(Time::S) - m_estimatedRtt.ToDouble(Time::S);

  //m_CurrentDelta = Time::FromDouble(delta,Time::S);
...
RttMeanDeviation::IntegerUpdate (Time m, uint32_t rttShift, uint32_t variationShift)
{
 ...
 /*USED TO RETURN CURRENT DELTA*/
  m_CurrentDelta = Abs(Time::From(delta));
...
RttMeanDeviation::Measurement (Time m)
{
  ...
  if (m_nSamples)
    ...
    double numerator = 0.0;     //upper part of the fraction
    double denominator = 0.0;   //lower part of the fraction


    /********************Prediction**********************/

    //calculating the expert prediction rtt estimate (y_t)
    for(int i=0;i<100;i++){
      numerator = numerator + m_weights[i] * m_expert_predictions[i];   //sum of (weight_i * x_i)

      //NS_LOG_INFO("numerator = "<<numerator<<", m_weights[i] = "<<m_weights[i]<<", m_expt[i] = "<<m_expert_predictions[i]);


      denominator = denominator + m_weights[i];                         //sum of weight_i
    }

    double y_t = numerator/denominator;

    m_prediction = Time::FromDouble(y_t, Time::S);

    //NS_LOG_INFO("Prediction = "<<m_prediction.ToDouble(Time::S)<<" at sample = "<<m_nSamples);
    ...
    for(int i=0;i<100;i++){
      ...
      if( m_expert_predictions[i] >= m.ToDouble(Time::S) ){

        m_loss_function_values[i] = (m_expert_predictions[i] - m.ToDouble(Time::S)) * (m_expert_predictions[i] - m.ToDouble(Time::S));
      
      }
      else{

        m_loss_function_values[i] = 2 * m.ToDouble(Time::S);
    ...
    for(int i=0;i<100;i++){
      ...
      double exp = pow( e , -(m_learning_rate * m_loss_function_values[i]) );     //e ^ - (iota * L_i,t(x_i,y_t)
      
      m_exponential_updates[i] = m_weights[i] * exp;
    ...
    //calcualate value of POOL
    double pool = 0.0;

    for(int i=0;i<100;i++){

      pool = pool + m_expert_alpha * m_exponential_updates[i];
    ...
    for(int i=0;i<100;i++){
      ...
      m_weights[i] = (1-m_expert_alpha)*m_exponential_updates[i] + ( pool / (i+1) );

    }

    //m_CurrentDelta = Abs(m_prediction - m);
    ...
    { // First sample
      ...
      for(int i=0;i<100;i++){
        m_expert_predictions[i] = m_minRTT + ( m_maxRTT * pow(2, (i+1)-100)/4 ); 
      }

      m_prediction = m;
      //m_CurrentDelta = Time::From(0);
